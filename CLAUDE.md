...
- **`make`** — installs deps (uv + yarn) if needed, creates DB, then runs Django dev server and Webpack watch concurrently
- **`uv run python manage.py runserver`** — Django only
- **`npx webpack --config webpack.config.js --mode development --watch --devtool source-map`** — Webpack only
...
- **Deps:** Python via `uv`, JS via `yarn`. No tests, no linters.
...
| App | Purpose |
|---|---|
| `analytics` | Project config (settings, URLs, templates, ASGI/WSGI, context processors, Playwright PDF utils). Webpack outputs land in `analytics/static/`. |
...
**Production:** Single Docker container running Gunicorn with Uvicorn workers (ASGI). Caddy as reverse proxy. Data persisted to `/srv/data/analytics/`. Deployed via `git push server master` triggering a post-receive hook.
...
- Playwright + Chromium are bundled in the Docker image for server-side PDF generation.
